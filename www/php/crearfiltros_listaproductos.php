...
<head>
  ...
  <title>Registro de Filtros de Lista de Productos y HTS</title>
  ...
  <script type="text/javascript">
     $(document).ready(function ($) {
        ...
        $('#nuevo').on('click', function(){
          accion="nuevo";
          arreglar_ventana();
          $('#form_listaproducto').each (function(){
            this.reset();
          });
          $('#myModal').modal('show');
        });
       ...
       //codigo que se ejecuta al editar un nuevo elemento
        $('#tabla tbody').on( 'click', '#editar', function () {
            ...
            $(this).parents('tr').each(function (index) 
            {
              $(this).children("td").each(function (index2) 
              {
                switch (index2) 
                {
                    case 0: $('#myModal').find('#nombre_listaproducto').val($(this).text());
                            break;
                    case 1: $('#myModal').find('#hts_listaproducto').val($(this).text());
                            break;
                    case 2: $('#myModal').find('#nandina').val($(this).text());
                            break;
                    case 3: $('#myModal').find('#costo_listaproducto').val($(this).text());
                            break;
            ...
            $('#myModal').find('#id_listaproducto').val(id_fila);
            $('#myModal').modal('show');
       });
        ...
        $('#tabla tbody').on( 'click', '#eliminar', function () {
            ...
            $('#EliminarModal').find('#eliminar_id_listaproducto').val(id_fila)
            $('#EliminarModal').modal('show');
        });
        //codigo que se ejecuta al dar si en el eliminar
        $('#elim_listaproducto').on('click', function () {
            
             var parametros = {
                "id":$('#eliminar_id_listaproducto').val(),
                "accion":accion
              };
              $.ajax({
                    data:  parametros,
                    url:   'nuevofiltro_listaproducto.php',
                    type:  'post',
                    dataType: 'json',
    ...
              });
        ...
        $("#form_listaproducto").validate({
            errorClass: "my-error-class",
            validClass: "my-valid-class",
    
            // Specify the validation rules
            rules: {
                  nombre_listaproducto: {required:true},
                  hts_listaproducto: {required:true},
                  nandina: {required:true},
                  costo_listaproducto: {
                    required: true,
                    noSpace: true,
                    number: true
        ...
        messages: {
              nombre_listaproducto: "Campo requerido",
              hts_listaproducto: "Campo requerido",
              nandina: "Campo requerido",
              costo_listaproducto: {
        ...
        submitHandler: function(form) {
           ...
           {
             var parametros = {
                "nombre" : $('#nombre_listaproducto').val(),
                "hts" : $('#hts_listaproducto').val(),
                "nandina" : $('#nandina_listaproducto').val(),
                "costo" : $('#costo_listaproducto').val(),
                "accion":accion
             };
             $.ajax({
                    data:  parametros,
    ...
                    success:  function (response) {
                        ...
                        $('#mensaje').fadeIn('slow',function (){
                         ...
                         var cell1 = oTable.cell($('tr#'+id_fila+' td:eq(1)'));
                             cell1.data(response.hts).draw();
                         var cell2 = oTable.cell($('tr#'+id_fila+' td:eq(2)'));
                             cell2.data(response.nandina).draw();
                         var cell3 = oTable.cell($('tr#'+id_fila+' td:eq(3)'));
                             cell3.data(response.costo).draw();
...
<div class="panel-body" align="center">
  ...
  <form action="" method="post" target="">
    ...
    <div class="col-md-10">
    	...
    	<h3><strong>Listado de Producto</strong></h3>
    </div>
</div>
    
     <table id="tabla" cellspacing="0" width="100%" class="display" >  
        <thead>
        <th><strong>Lista de Productos</strong></th>
        <th><strong>HTS</strong></th>
        <th><strong>Nandina</strong></th>
        <th><strong>Costo Decl/Stem</strong></th>
       ...
       <tbody>
            <?php
            //Leer todas las listaproducto
            $sql = "SELECT * FROM tbllistaproducto";
            $val = mysqli_query($link, $sql);
             if(!$val){
                ...
                while($row = mysqli_fetch_array($val))
                {
                  ...
                  echo "<td >".$row['HTS']."</td>";
                  echo "<td >".$row['nandina']."</td>";
                  echo "<td >".$row['costo_Decl_Stem']."</td>";
                  echo '<td ><img id="editar" src="../bootstrap/fonts/glyphicons/glyphicons/glyphicons-151-edit.png" data-toggle="tooltip" data-placement="left" title = "Modificar Lista de Producto"/></td>';
		  echo '<td ><img id="eliminar" src="../bootstrap/fonts/glyphicons/glyphicons/glyphicons-17-bin.png" data-toggle="tooltip" data-placement="left" title = "Eliminar Lista de Producto"/></td>';
                  echo "</tr>";
	        }
...
<!--Ventana modal para crear nuevo listaproducto-->
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
          <form id="form_listaproducto" method="post">
          <div class="row">
            <div class="col-md-8">
              <label for="nombre_listaproducto" class="control-label">Nombre:</label>
              <input type="text" class="form-control" name="nombre_listaproducto" id="nombre_listaproducto"/>
            </div>
          </div>  
          <div class="row">
             <div class="col-md-4">
                 <label for="hts_listaproducto" class="control-label">HTS:</label>
                 <input type="text" class="form-control" name="hts_listaproducto" id="hts_listaproducto"/>
             </div>
         </div>
         <div class="row">
             <div class="col-md-4">
                 <label for="nandina_listaproducto" class="control-label">Nandina:</label>
                 <input type="text" class="form-control" name="nandina_listaproducto" id="nandina_listaproducto"/>
             </div>
         </div>
          <div class="row">
            <div class="col-md-4">
                 <label for="costo_listaproducto" class="control-label">Costo Decl/Stem:</label>
                 <input type="text" class="form-control" name="costo_listaproducto" id="costo_listaproducto"/>
             </div>
          </div>
          <input type="hidden" name="id_listaproducto" id="id_listaproducto" />
...
<div class="modal fade" id="EliminarModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="exampleModalLabel">Eliminar Producto y HTS</h4>
      </div>
      <div class="modal-body">
          <form id="form_listaproducto" action="nuevofiltro_listaproducto.php">
          <div class="form-group">
            <label for="nombre_listaproducto" class="control-label">Está Seguro que desea eliminar este Producto</label>
          </div>
          <input type="hidden" name="eliminar_id_listaproducto" id="eliminar_id_listaproducto" />
          </form>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">No</button>
        <button type="button" id="elim_listaproducto" onclick="" class="btn btn-primary">Si</button>
      </div>
    </div>
  </div>
</div>
