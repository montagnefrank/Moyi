...
<head>
  ...
  <title>Registro de Filtros de Mediatallos</title>
  ...
  <script type="text/javascript">
     $(document).ready(function ($) {
        ...
        $('#tabla tbody').on( 'click', '#editar', function () {
            ...
            $('#myModal').find('#id_mediatallo').val(id_fila)
            $('#myModal').find('#nombre_mediatallo').val(valor);
            $('#myModal').modal('show');
        });
        ...
        $('#tabla tbody').on( 'click', '#eliminar', function () {
            ...
            id_fila=$(this).closest('tr').attr('id');
            valor=$(this).parents('tr').children('td:eq(0)').html();
            $('#EliminarModal').find('#eliminar_id_mediatallo').val(id_fila)
            $('#EliminarModal').modal('show');
        });
        
        //codigo que se ejecuta al dar si en el eliminar
        $('#elim_mediatallo').on('click', function () {
           var parametros = {
                "id":$('#eliminar_id_mediatallo').val(),
                "accion":accion
              };
              $.ajax({
                    data:  parametros,
                    url:   'nuevofiltro_mediatallo.php',
                    type:  'post',
                    dataType: 'json',
    ...
              });  
        ...
        $("#form_mediatallo").validate({
            errorClass: "my-error-class",
            validClass: "my-valid-class",
    
            // Specify the validation rules
            rules: {
                  nombre_mediatallo: {required:true},
            },
...
             };
    ...
                    success:  function (response) {
                        ...
                        $('#mensaje').fadeIn('slow',function (){
                           ...
                           response.nombre,
                           '<img id="editar" src="../bootstrap/fonts/glyphicons/glyphicons/glyphicons-151-edit.png" data-toggle="tooltip" data-placement="left" title = "Modificar Mediatallo"/></td>',
		           '<img id="eliminar" src="../bootstrap/fonts/glyphicons/glyphicons/glyphicons-17-bin.png" data-toggle="tooltip" data-placement="left" title = "Eliminar Mediatallo"/></td>'
                           ]).draw(false).index();
...
<div align="center" width="100%">
 ...
 <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
    <ul class="nav navbar-nav">
               <?php
			  if($rol == 3 || $rol == 8){?>
			  	<li><a href="../php/subirTracking.php"><strong>Cargar Tracking</strong></a></li>
			  <?php }?> 
...
<div class="panel-body" align="center">
  ...
  <form action="" method="post" target="">
    ...
    <div class="col-md-10">
        <div class="col-md-2">
            <button type="button" class="btn btn-primary" id="nuevo" data-toggle="tooltip" data-placement="rigth" title = "Crear nuevo Mediatallo">
                <span class="glyphicon glyphicon-plus-sign black"></span>
            </button>
      </div>
    	<h3><strong>Listado de Mediatallos</strong></h3>
    </div>
</div>
     ...
     <table id="tabla" cellspacing="0" width="100%" class="display" >  
       ...
       <tbody>
            <?php
            //Leer todas las mediatallos
            $sql = "SELECT * FROM tblmediatallos";
            $val = mysqli_query($link, $sql);
             if(!$val){
                ...
                while($row = mysqli_fetch_array($val))
                {
                  ...
                  echo '<td ><img id="editar" src="../bootstrap/fonts/glyphicons/glyphicons/glyphicons-151-edit.png" data-toggle="tooltip" data-placement="left" title = "Modificar Mediatallo"/></td>';
		  echo '<td ><img id="eliminar" src="../bootstrap/fonts/glyphicons/glyphicons/glyphicons-17-bin.png" data-toggle="tooltip" data-placement="left" title = "Eliminar Mediatallo"/></td>';
                  echo "</tr>";
	        }
...
<!--Ventana modal para crear nuevo mediatallo-->
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="exampleModalLabel">Nuevo Mediatallo</h4>
      </div>
      <div class="modal-body">
          <form id="form_mediatallo" action="nuevofiltro_mediatallo.php">
          <div class="form-group">
            <label for="nombre_mediatallo" class="control-label">Tamaño(cm):</label>
            <input type="text" class="form-control" name="nombre_mediatallo" id="nombre_mediatallo"/>
          </div>
          <input type="hidden" name="id_mediatallo" id="id_mediatallo" />
...
<div class="modal fade" id="EliminarModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="exampleModalLabel">Eliminar Mediatallo</h4>
      </div>
      <div class="modal-body">
          <form id="form_mediatallo" action="nuevofiltro_mediatallo.php">
          <div class="form-group">
            <label for="nombre_mediatallo" class="control-label">Está Seguro que desea eliminar este Mediatallo</label>
          </div>
          <input type="hidden" name="eliminar_id_mediatallo" id="eliminar_id_mediatallo" />
          </form>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">No</button>
        <button type="button" id="elim_mediatallo" onclick="" class="btn btn-primary">Si</button>
      </div>
    </div>
  </div>
</div>
