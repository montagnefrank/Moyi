		...
		while($j <= $fila){ //Aqui recorro cada una de las filas leida de las ordenes
				 ...
				 if($Ponumber == ''  ||  $CustNumber == '' || $Guia_madre == ''  ||  $Guia_hija == ''){
					...
					$j++;
                                        ...
                                        $row = mysql_query($query,$conection) or die ("Error verificando si la orden es un reenvio");				
                                        $ray = mysql_num_rows($row); //cuento las filas devueltas
						 ...
						 $row = mysql_query($query,$conection) or die ("Error verificando si el tracking existe");				
						 $ray = mysql_num_rows($row); //cuento las filas devueltas
						 ...
						 if ($ray != 0 ){
							 ...
							 $j++;
                                                   ...
                                                   $row = mysql_query($query,$conection) or die ("Error verificando si la orden existe");				
                                                   $ray = mysql_num_rows($row); //cuento las filas devueltas
                                                   ...
                                                   else
                                                   {
                                                        ...
                                                        if($tracking == ''){
                                                            ...
                                                            else
                                                            {
                                                               ...
                                                               $ejecutar = mysql_query($consulta,$conection) or die ("Error insertando el código único"); 
                                                            }      

                                                           //Actualizar la orden con los datos de la finca y caja
                                                           $sql11="Update tbldetalle_orden Set tracking='$Tracking', status = 'Shipped',descargada='Downloaded', farm='$finca', codigo='$codigo' where id_orden_detalle = '$id_order'"; // actualizar el eBing ='$eBing',
                                                           $actualizado = mysql_query($sql11,$conection);
                                                           
                                                           $fecha = date('Y-m-d');
                                                           
                                                           $sql="INSERT INTO tblcoldroom
                                                            (codigo,entrada,item, finca,fecha,salida,fecha_tracking,guia_madre,guia_hija,fecha_entrega,servicio,fecha_vuelo,tracking_asig,airline,rechazada,guia_master,palet)
                                                             VALUES ('$codigo','Si',$item','$finca','$fecha','Si','$fecha','$Guia_madre','$Guia_hija','$entrega','$servicio','$vuelo','$Tracking','$aerolinea','0','C','0')";				 			 										
                                        ...
                                        //insertar nuevo tracking
                                        $query = "select tracking, Ponumber, Custnumber, cpitem from tbldetalle_orden where tracking = '$Tracking'";			 
                                        $row = mysql_query($query,$conection) or die ("Error verificando el tracking");				
                                        $ray = mysql_num_rows($row); //cuento las filas devueltas
                                            ...
                                            if ($ray == 0 ){
                                               ...
                                               for($i= 0; $i < $ray; $i++){
                                                    ...
                                                    if($tracking == ''){
                                                        ...
                                                        $fecha = date('Y-m-d');
                                                        //Crear una entrada al cuarto frio de las finca
